    ...
    <div class="mt-4">
      ...
      <?php if ($payment_status): ?>
        <?php
          ...
          } elseif (strtolower($payment_status) == 'failed') {
              $payment_status_class = 'text-danger';
          } elseif (strtolower($payment_status) == 'pending') {
              $payment_status_class = 'text-warning';
          }
        ?>
        <p class="<?php echo $payment_status_class; ?>">
          <strong>Last payment status:</strong> <?php echo htmlspecialchars($payment_status); ?><br>
          <strong>Payment Date:</strong> <?php echo date('F j, Y, g:i a', strtotime($payment_date)); ?>
        </p>
      <?php else: ?>
        <p class="text-muted">No payment made yet. Please complete your payment to access exclusive content.</p>
      <?php endif; ?>
    </div>
